...
void normalizeLyap();
void modulePrevalence(double*, double*);
...
//array of which module has the biggest lyapunov contribution
double largestModArr[NUM_LYAPS] = {};

//array of mu values
double muValues[10] = {3.1, 3.3, 3.5, 3.55, 3.555, 3.559, 3.56, 3.561, 3.565, 3.57};
//double muValues[10] = {3.6, 3.8, 4.0}
...
int main(int argc, char *argv[]) /**/
{
    ...
    { // runs the entire code for each percent internal (P) value
        ...
        { // basically runs the entire code for each network structure
            ...
            string inputStructFileName = "./data/P" + patch::to_string(percentIntConn) + "inputstruct" + patch::to_string(randNetwork) + ".txt";
            ...
            for (int timeStep = 0; timeStep < NUM_TIMESTEPS; timeStep++)
            {
                ...
                modulePrevalence(largestContrModPt, largestModPt);
            ...
            for (int lyapNum = 0; lyapNum < NUM_LYAPS; lyapNum++)
            {
                ...
                outFileMod << largestContrModLong << " ";
                outFileModSquared << largestContrModLongSquared << " ";
            } /**/
...
// this part calculates the coarse-grained participation ratios        
void modulePrevalence(double *largestContrMod, double *largestMod)
{
        for (int lyapNum = 0; lyapNum < NUM_LYAPS; lyapNum++)
        {
		...
		*activeLargestContrMod = 0;
	       	...
	       	double *activeLargestMod = largestMod + lyapNum;
                *activeLargestMod = 0;
		...
		for (int activeMod = 0; activeMod < NUM_MODULES; activeMod++)
		{
			...
			if (modTotalContr > maxModTotalContr)
			{
				  ...
				  //*activeLargestMod = activeMod;
			}
			cout << maxModTotalContr << endl;
			maxModTotalCheck += modTotalContr;
//*activePartRatioCoarse += lyapContrCoarse * lyapContrCoarse;
		}
		cout << maxModTotalCheck << endl;
		*activeLargestContrMod = maxModTotalContr;
	}
}
