...
int main()
{
	...
	/*if (argc != 4)
	{
		cout << "Intended command line inputs: .\name, Dlow, Dhigh, Dinterval"
			<< endl;
		return -1;
	}

	double DFactor = atof(argv[1]) + 0.01 * atof(argv[3]); /**/
	...
	{ // basically runs the entire code for each network structure
		...
		string trialFileName = "mu" + patch::to_string(MU_FACTOR)
...
int loadNetworkStructure(string inputFileName)
{
    ...
    //paul
    string line;
    ifstream myfile (inStructFileName);
    if (myfile.is_open())
    {
