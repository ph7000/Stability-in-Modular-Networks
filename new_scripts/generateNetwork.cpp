...
/* const double
NUM_TOTAL_CONN = 6.0,
NUM_INT_CONN = NUM_TOTAL_CONN / 2.0,
NUM_EXT_CONN = NUM_TOTAL_CONN - NUM_INT_CONN,
  PROB_INT_CONN = (NUM_INT_CONN / (NODES_PER_MODULE - 1)), // (3.2 / 7.0),
  PROB_EXT_CONN = (NUM_EXT_CONN / (NUM_NODES - NODES_PER_MODULE)), // (0.8 / 56.0),
  PERCENT_INT_CONN = (NUM_INT_CONN / NUM_TOTAL_CONN);/**/

const bool DIFFERENT_NUM_NODES = false;
const bool DIFFERENT_NUM_TOTAL_CONN = true;
...
// arrays of connections per node
double kValues[1] = {8};
...
//int main()
{
    ...
    int randSeed = (int)atof(argv[1]); // + atof(argv[3]); /**/
    
    // Making networks according to how many k or P values there are, and iterating over all of these values.
    ...
    for (int PPosition = 0; PPosition < iterator; PPosition++)
    { // runs the entire code for each percent internal (P) value or each degree (k) value.
      
        // Depending on the size of the arrays of k and P values, we iterate over one
        if(kSize == 1){ // then we iterate over P values
            degree = kValues[0];
            percentIntConn = PValues[PPosition];
        } else if (PSize == 1) { // then we iterate over k values
            percentIntConn = PValues[0];
            degree = kValues[PPosition];
        }
        
        double
        numIntConn = degree*percentIntConn,
        numExtConn = degree - numIntConn,
        probIntConn = (numIntConn / (NODES_PER_MODULE - 1)), // (3.2 / 7.0),
        probExtConn = (numExtConn / (NUM_NODES - NODES_PER_MODULE)); // (0.8 / 56.0),
        /**/
        // sets random seed
        srand((PPosition + 1)*randSeed);

        // creates network. If not all nodes have connections, try again.
        ...
        cout << modCalc() << endl; // prints the modularity. This is different than the percent of internal connections.

        cout << "done" << endl;

        fstream outFileStruct;
        string structFileName = //"inputstruct" + patch::to_string(randSeed)
        "../NetworkSimFixed/data/P" + patch::to_string(percentIntConn);
            
        // making directories to hold the network structures.
        if (mkdir(structFileName.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) == -1)
          {
            if( errno == EEXIST ) {
              // already exists
            } else {
        ...
        structFileName += "/M" + patch::to_string(NUM_MODULES);
        ...
        structFileName += "/N" + patch::to_string(NODES_PER_MODULE);
        ...
        structFileName += "/P" + patch::to_string(percentIntConn)
        ...
        structFileName += "/inputstruct" + patch::to_string(randSeed) + ".txt";
        outFileStruct.open(structFileName.c_str(), fstream::out);

        showConnectionsBoolean(outFileStruct);
    
    } // end of for loop over P or k values
...
// nodes in the module with probability probIntConn
