...
const bool DIFFERENT_NUM_TOTAL_CONN = false;
...
double kValues[9] = {2, 3, 4, 5, 6, 8, 12, 16, 20};

// array of number of nodes/number of modules combinations
double nodeValues[9][2] = {{4, 4},{4, 8},{8, 4},{6, 6},{2, 8},{8, 2},{12, 12},{6, 12},{12, 6}};
...
//int main()
{
  ...
  int networkValue = onesDigit + 10*tensDigit;

  int networkNumber = (networkValue + 1);

  if (DIFFERENT_NUM_NODES || DIFFERENT_NUM_TOTAL_CONN)
    {
  ...
  if (argSize == 3)
    {
      vectorPosition = (commandArg - networkValue)/100;
    }

  if (DIFFERENT_NUM_NODES)
    {
      numModules = nodeValues[vectorPosition][0];
      numNodesPerModule = nodeValues[vectorPosition][1];
      numNodes = numModues*numNodesPerModule;
      cout << "The size of the command argument is " << argSize << endl;
      cout << numNodes << " <- number of nodes, network number -> " << networkNumber << endl;
    }

  if (DIFFERENT_NUM_TOTAL_CONN)
    {
      degree = nodeValues[vectorPosition];
      cout << "The size of the command argument is " << argSize << endl;
      cout << degree << " <- degree, network number -> " << networkNumber << endl; /**/
    }
...
/*} else {
  ...
  int maxDistance = numNodes - 1;/**/
	...
	probIntConn = (numIntConn / (nodesPerModule - 1)), // (3.2 / 7.0),
	probExtConn = (numExtConn / (numNodes - nodesPerModule)), // (0.8 / 56.0),
	/**/
    // sets random seed
    ...
    string structFileName = //"inputstruct" + patch::to_string(randSeed)
      "../NetworkSimFixed/data/"
      + "P" + patch::to_string(PERCENT_INT_CONN);
    
    if (DIFFERENT_NUM_NODES || DIFFERENT_NUM_TOTAL_CONN)
      {
	structFileName += "/Mo" + patch::to_string(numModules)
	  + "/" + "No" + patch::to_string(nodesPerModule)
	  + "/k" + patch::to_string(degree)
	  + "/P" + patch::to_string(percentIntConn)
	  + "Mo" + patch::to_string(numModules)
	  + "No" + patch::to_string(nodesPerModule)
	  + "k" + patch::to_string(degree) + "/";
      }

    structFileName += "inputstruct" + patch::to_string(networkNumber) + ".txt";
    outFileStruct.open(structFileName.c_str(), fstream::out);

    showConnectionsBoolean(outFileStruct);
    
    } // end of for loop over P values
